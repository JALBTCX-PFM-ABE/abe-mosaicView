  ...
  QDialog (pt, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  if (new_feature)
    {
      ...
      bfd_record.feature_type = BFDATA_INFORMATIONAL;
    }
  else
    {
  ...
  QString recordID;
  recordID = QString ("%1").arg (bfd_record.record_number, 5, 10, QChar ('0'));
  ...
  QLabel *featureType = new QLabel (typeBox);
  featureType->setToolTip (tr ("This is the feature type for the current feature)"));
  featureType->setWhatsThis (featureTypeText);
  typeBoxLayout->addWidget (featureType);
  idBoxLayout->addWidget (typeBox);

  if (new_feature || bfd_record.feature_type == BFDATA_INFORMATIONAL)
    {
      featureType->setText (tr ("Informational"));
    }
  else
    {
      featureType->setText (tr ("Hydrographic"));
    }
  ...
  QString depth;
  depth = QString ("%L1").arg (bfd_record.depth, 0, 'f', 2);
...
editFeature::slotViewError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("mosaicView image viewer"), tr ("There was a write error to the image viewer process!"));
      break;

    case QProcess::ReadError:
      QMessageBox::critical (this, tr ("mosaicView image viewer"), tr ("There was a read error from the image viewer process!"));
      break;

    case QProcess::UnknownError:
      QMessageBox::critical (this, tr ("mosaicView image viewer"), tr ("The image viewer process died with an unknown error!"));
      break;
    }
}
...
editFeature::slotSnippetViewerReadyReadStandardError ()
{
  ...
  fprintf (stderr, "%s %s %s %d %s\n", misc->progname, __FILE__,  __FUNCTION__, __LINE__, res);
  fflush (stderr);
}
          ...
          if (fd.exec () == QDialog::Accepted)
            {
              ...
              if ((misc->bfd_handle = binaryFeatureData_open_file (filename, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                {
                  QMessageBox::warning (parent, "mosaicView", tr ("Creating new feature file not allowed in mosaicView, use pfmView."));
                  return;
                }
              else
                {
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_open_file (misc->abe_share->open_args[0].target_path, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                    {
                      ...
                      QMessageBox::warning (parent, "mosaicView", tr ("Unable to open feature file\nReason: %1").arg (msg));
                      return;
                    }
...
editFeature::slotDelete ()
{
  ...
  if (QMessageBox::information (this, "mosaicView", 
...
editFeature::slotGroup ()
{
      ...
      if (inside (bfd_polygon.longitude, bfd_polygon.latitude, bfd_record.poly_count, misc->feature[i].longitude,
                  misc->feature[i].latitude))
        {
          if (misc->feature[i].parent_record)
            {
              QMessageBox::warning (this, "mosaicView", tr ("A sub-feature already exists within the polygon.\nPlease un-group the features first."));
              return;
            }

          if (misc->feature[i].child_record)
            {
              QMessageBox::warning (this, "mosaicView", tr ("A master feature already exists within the polygon.\nPlease un-group the features first."));
              return;
            }
        }
    }
  ...
  if (QMessageBox::information (this, "mosaicView", tr ("Grouping sub-features forces an apply of all changes to this record.  Do you really want to do this?"),
                                QMessageBox::Yes,  QMessageBox::No,  QMessageBox::NoButton) == QMessageBox::No) return;
  ...
  if (binaryFeatureData_write_record (misc->bfd_handle, bfd_record.record_number, &bfd_record, &bfd_polygon, NULL) < 0)
    {
      fprintf (stderr, "%s %s %s %d - array - %s\n", misc->progname, __FILE__, __FUNCTION__, __LINE__, binaryFeatureData_strerror ());
      fflush (stderr);
    }
  ...
  uint32_t prev_rec = rec.record_number + 1;
      ...
      if (misc->feature_polygon_flag > 0)
        {
          ...
          double max_dist = -1.0, max_az = 0.0;
          double dist, az;

          for (int32_t i = 0 ; i < misc->poly_count ; i++)
            {
              for (int32_t j = 0 ; j < misc->poly_count ; j++)
                {
                  if (j != i)
                    {
